...
| v40.0.0 (Phase D complete) | v40.0.0 introduces the Genesis Workspace Root: a persistent BogFS workspace whose user-facing paths are mutable, but whose trusted state is an append-only chain of deterministic root transitions and verifier receipts. Phase D: GenesisRoot persisted as well-known object in existing v37/v38 manifest; boot/mount validation + receipt-chain survival proven (oracle + image mutation + kernel load). | v36-v39 |
| v41 (journal started) | Native workspace journal receipts: multi-operation append-only hash-chained journal of WorkspaceReceipts (ledger_root in GenesisRoot = journal head). Kernel-native append/rollback/chain-verify/ history inspection using pure model. Rollback appends (history preserved). Undeniable: tamper breaks chain on load. See v40 doc. | v40 |
