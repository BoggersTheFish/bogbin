...
BOGPK-0.1 adds the first binary-packed container path. It enum-packs transforms and bases, derives chunk offsets implicitly, delta-codes residual offsets, and supports zero-residual runs. The current `.bogpk` mean container/input ratio is `1.419816`, down from the JSON `.bog` ratio of `38.548519`, but the full fixture set still does not cross the compression threshold.
